...
def process_muscle_up_video(video_path, height_cm, weight_kg, start_time=1.0, output_video_path='output_video.mp4'):
    ...
    while video.isOpened():
        ...
        if results.pose_landmarks:
            ...
            shoulder = landmarks[mp_pose.PoseLandmark.RIGHT_WRIST]
            ...
            # 显示 Beast 概率（全程显示）
            # beast_text = f"Velocity_Change: {beast_probability:.1f}%"
            #cv2.putText(frame, beast_text, (80, 160), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 3)
            if velocity_log:  # 确保数组非空，防止报错
                velocity_text = f"Velocity: {velocity_log[-1]['velocity']:.1f}m/s"
            else:
                velocity_text = "Velocity: N/A"
            cv2.putText(frame, velocity_text, (80, 160), cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0, 255, 255), 3)
            ...
            if frame_num >= start_frame and prev_shoulder_y is not None:
                ...
                if delta_y < 0 and abs(velocity) > 0.1 and ascend_start_frame is None:
    ...
    df.to_csv('velocity_shoulderpress_data.csv', index=False)  # 保存为 CSV
    # df.to_excel('velocity_data.xlsx', index=False)  # 或保存为 Excel
    print("数据已保存")
    ...
    # 4. 显示图形
    plt.show()
    plt.savefig('velocity_time_plot.png')

    return muscle_up_data

# 示例用法
if __name__ == "__main__":
    video_path = 'D:/Users/G122/PycharmProjects/pythonProject6/SP/SP_in.mp4'
    height_cm = 173  # 替换为你的身高（厘米）
    weight_kg = 64   # 替换为你的体重（千克）
    output_video_path = 'D:/Users/G122/PycharmProjects/pythonProject6/SP/6_vertical_sp_analysis.mp4'

    try:
        muscle_up_results = process_muscle_up_video(video_path, height_cm, weight_kg, start_time=0, output_video_path=output_video_path)
